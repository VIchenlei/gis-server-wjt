...
export default class User {
    ...
    socket.on('PUSH', (req) => {
      ...
      if (socket.auth) {
        ...
        if (cmd === 'event' || cmd === 'resp_all_data') {
          ...
          const powerLevels = this.getAllAcAlarms(events)
          if (powerLevels.length > 0) {
            for (let i = 0; i < powerLevels.length; i++) {
              const {power_levels_id, device_power_id, device_id} = powerLevels[i]
              this.power.updateTime(power_levels_id, device_power_id, device_id, 1)
            }
          }
  ...
  getAllAcAlarms (datas) {
    const powers = this.metaStore.data.power_levels && Array.from(this.metaStore.data.power_levels.values())
    let powerLevels = []
    for (let i = 0; i < datas.length; i++) {
      const data = datas[i]
      if (data.type_id === 44) {
        const objID = parseInt(data.obj_id, 10)
        const deviceID = this.metaStore.data.device_power && this.metaStore.data.device_power.get(objID).device_id
