...
const metadata = {
  ...
  card: {
    ...
    keyIndex: 0, // table中key值在 field 中的位置
    fields: {
      names: ['card_id', 'card_type_id', 'ident', 'state_id', 'comment', 'hw_version'], // 字段，card_id = card_type_id + card_ident
      types: ['NUMBER', 'SELECT', 'NUMBER', 'SELECT', 'STRING', 'STRING'], // 字段类型
      labels: ['卡号', '卡类型', '卡标识', '使用状态', '备注', '卡硬件版本'],
...
*/
  ...
  vehicle_category: {
    ...
    fields: {
      ...
      types: ['NUMBER', 'STRING', 'STRING', 'SELECT', 'NUMBER'],
      labels: ['车辆类别编号', '类型名称', '类型描述', '类型颜色', '超速速度'],
      enableNull: [false, false, true, false, true]
    }
  },
  vehicle_level: {
    name: 'vehicle_level',
  ...
  */
  ...
  reader: {
    ...
    keyIndex: 0, // table中key值在 field 中的位置
    fields: {
      names: ['reader_id', 'reader_type_id', 'name', 'brief_name', 'device_power_id', 'map_id', 'area_id', 'x', 'y', 'z', 'angle', 'state', 'ip', 'device_type_id', 'dimension', 'enable_simulate_card', 'need_power_alarm', 'fade_value', 'vehicle_id'], // 字段
      types: ['NUMBER', 'SELECT', 'STRING', 'STRING', 'SELECT', 'SELECT', 'SELECT', 'NUMBER', 'NUMBER', 'NUMBER', 'NUMBER', 'SELECT', 'STRING', 'SELECT', 'SELECT', 'SELECT', 'SELECT', 'NUMBER', 'SELECT', 'SELECT'], // 字段类型
      labels: ['分站编号', '分站类型', '分站名称', '分站简称', '电源编号', '所属地图', '所属区域', '坐标X', '坐标Y', '坐标Z', '分站仰角', '分站状态', 'IP', '设备类型', '定位维度', '是否可以模拟成定位卡', '掉电告警', '盲区阈值', '绑定车辆'],
      enableNull: [false, false, false, true, true, false, false, false, false, true, false, false, true, false, false, false, false, false, true]
  ...
  */
  ...
  device_power: {
    ...
    fields: {
      ...
      enableNull: [false, false, false, false, false, false, false, false, false, false, false, false, true, true, true]
    }
  },
  power_levels: {
    name: 'power_levels',
    ...
    fields: {
      ...
      labels: ['编号', '电路编号', '电源编号', '电池编号', '电池型号', '电池类型', '额定电压', '放电截止电压', '标称容量(mAH)', '强制放电周期(d)', '标准放电时间(h)'],
  ...
  light_shape: {
    name: 'light_shape',
    label: '红绿灯形状管理',
    table: 'dat_light_shape',
    keyIndex: 0,
    fields: {
      names: ['light_shape_id','shape_name', 'shape_code'],
  ...
  light_rd_spec_process: {
    ...
    label: '红绿灯分站管理',
    table: 'dat_light_rd_spec_process',
    keyIndex: 0,
    fields: {
      names: ['id', 'reader_id','light_id', 'reader_state', 'shape_id'],
      types: ['NUMBER', 'NUMBER', 'NUMBER', 'SELECT', 'SELECT'],
      labels: ['编号', '分站编号', '红绿灯编号', '分站状态', '红绿灯形状'],
      enableNull: [false, false, false, false, false, false]
    }
  },
}
